...
import { RECEPTION_TOKEN_COOKIE } from "./types";
...
export async function stayFetch<T>(path: string, opts: StayFetchOptions = {}): Promise<T> {
  ...
  const host = opts.host || process.env.STAY_RECEPTION_HOST || "app.stay.hr";
  headers.Host = host.split(":")[0].toLowerCase();
  ...
  if (!res.ok) {
    ...
    throw new Error(`Stay API ${res.status}: ${text.slice(0, 300)}`);
  }
